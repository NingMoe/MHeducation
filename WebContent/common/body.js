...
function validateInputList(inputList) {
	for(var i=0; i<inputList.length; i++) {
		if(inputList.eq(i).val() == "") {
			var msg="";
			if(inputList.eq(i).parent().parent().find("span").html()){				
				msg = inputList.eq(i).parent().parent().find("span").html().trim() + "不能为空";
			}else{
				msg = inputList.eq(i).parent().parent().find("label").html().trim() + "不能为空";	
			}
			msg=msg.replace(/\s/g,"");
 ...
 * 验证身份证是否合法
 */
function isIdNumber(inputString) {
	var partten = /^\d{17}[\dXx]{1}$/;
	if (partten.test(inputString)) {
		return true;
	} else {
		return false;
